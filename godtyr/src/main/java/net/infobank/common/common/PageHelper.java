...
public class PageHelper {
    ...
    private int startRowNo; //시작 로우 번호    
    private int pageBtnSize; // 페이징 버튼 갯수
    ...
    private void makePaging() {
        ...
        if (this.pageBtnSize == 0) this.setPageBtnSize(5); // 기본 값 설정
        
        
        int rowNo =  (this.pageNo - 1) * this.pageSize; //페이지 시작 row
        ...
        int startPage = ((pageNo - 1) / this.pageBtnSize) * this.pageBtnSize + 1; // 시작 페이지 (페이징 네비 기준)
        int endPage = startPage + this.pageBtnSize - 1; // 끝 페이지 (페이징 네비 기준)
        ...
        if (isNowFirst) {
            ...
            //this.setPrevPageNo(((pageNo - this.pageBtnSize) < 1 ? 1 : (pageNo - this.pageBtnSize))); // 이전 페이지 번호            
            this.setPrevPageNo((startBtnNo - 1) < 1 ? 1 : (startBtnNo - 1)  ); // 이전 페이지 번호            
        }
        ...
        if (isNowFinal) {
            ...
            //this.setNextPageNo(((pageNo + this.pageBtnSize) > finalPage ? finalPage : (pageNo + this.pageBtnSize))); // 다음 페이지 번호
        	this.setNextPageNo((lastBtnNo + 1) > finalPage ? finalPage : (lastBtnNo + 1)); // 다음 페이지 번호
        }
