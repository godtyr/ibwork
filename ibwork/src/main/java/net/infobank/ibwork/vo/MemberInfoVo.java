...
public class MemberInfoVo implements UserDetails {
	...
	private static final long serialVersionUID = -5337610538915132770L;

	private int idx;
    private String id;                                  // 계정 아이디
    private String password;                            // 계정 비밀번호
    private String name;                                // 계정 사용자의 이름
    private String dept;                                // 계정 사용자의 이름
    private Set<GrantedAuthority> authorities;    // 계정이 가지고 있는 권한 목록     

    public MemberInfoVo(int idx, String id, String password, String name, String dept, Collection<? extends GrantedAuthority> authorities){
    	this.idx = idx;
        this.id = id;
